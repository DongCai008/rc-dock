...
class TabCache {
    ...
    render() {
        ...
        let tab = (React.createElement(DragDropDiv_1.DragDropDiv, { getRef: this.getRef, onDragStartT: onDragStart, role: "tab", "aria-selected": parent.activeId === id, onDragOverT: onDragOver, onDropT: onDrop, onDragLeaveT: onDragLeave },
            title,
            closeIcon && (React.createElement("button", { type: "button", className: "dock-tab-close-btn", onClick: this.onCloseClick }, closeIcon)),
            React.createElement("div", { className: "dock-tab-hit-area", ref: this.getHitAreaRef })));
        return (React.createElement(DockTabPane_1.default, { key: id, cacheId: id, cached: cached, tab: tab, closable: false, closeIcon: null }, content));
    }
    destroy() {
        // place holder
    }
...
class DockTabs extends React.PureComponent {
    ...
    render() {
        ...
        return (React.createElement(rc_tabs_1.default, { prefixCls: "dock", moreIcon: moreIcon, animated: animated, tabSeparator: separator, renderTabBar: this.renderTabBar, activeKey: activeId, editable: editable, onChange: this.onTabChange, popupClassName: classnames_1.default(Utils_1.groupClassNames(group)) }, children));
    }
}
